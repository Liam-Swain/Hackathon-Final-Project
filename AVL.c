...
Node* newNode(Stock sto)
{
    ...
    strcpy(temp->sto.change, sto.change);
...
Node* insertNodeByChange(Node* node, Stock sto)
{
    int balance = 0;
    ...
    if(strcmp(node->sto.change, sto.change) > 0)
        node->left = insertNodeByChange(node->left, sto);

    else if(strcmp(node->sto.change, sto.change) < 0)
        node->right = insertNodeByChange(node->right, sto);
    else
        return node;
    ...
    if(balance > 1 && strcmp(node->sto.change, node->left->sto.change) > 0)
        return rotateRight(node);

    if(balance < -1 && strcmp(node->sto.change, node->right->sto.change) < 0)
        return rotateLeft(node);
...
Node* insertNodeByTicker(Node* node, Stock sto)
{
    int balance = 0;
    ...
    if(strcmp(node->sto.ticker, sto.ticker) > 0)
        node->left = insertNodeByName(node->left, sto);

    else if(strcmp(node->sto.ticker, sto.ticker) < 0)
        node->right = insertNodeByName(node->right, sto);
    else
        return node;
    ...
    if(balance > 1 && strcmp(node->sto.ticker, node->left->sto.ticker) > 0)
        return rotateRight(node);

    if(balance < -1 && strcmp(node->sto.ticker, node->right->sto.ticker) < 0)
        return rotateLeft(node);
